window.addEventListener('WebComponentsReady', function() {
	...
	class AppPage{
		static selector(app_element,name){
			let pages = app_element.constructor.pages
			let selector = []
			if(pages){
				if('container' in pages) selector.push(pages.container)
				if('selector' in pages) selector.push(`[${pages.selector}="${name}"]`)
			}
			if(selector.length === 0) selector.push(name)
			return selector.join(' ').trim()
		}
		constructor(name,app_element){
			let pages = app_element.constructor.pages
			this.element = app_element.get_page_element(name)
			this.internal = is_internal(this.element)
			this.url  = get_url(this.element)
			
			//shared actions
			function get_url(element){
				let folder = get_folder()
				let prefix = 'prefix' in pages ? pages.prefix:''
				let suffix = 'suffix' in pages ? pages.suffix:''
				let items = [prefix,name,suffix]
				let value = clean_url()
				//return value
				return app_element.resolveUrl(`${folder}${value}.html`)
				
				//shared actions
				function get_folder(){
					let folder = ''
					if(element.hasAttribute('folder')) folder = element.getAttribute('folder')
					else folder = 'folder' in pages ? pages.folder:''
					return folder
				}
				function clean_url(){
				...
				//return app_element.resolveUrl('pages/' + page + '-page' + '.html')
			}
			
			function is_internal(element){
				let internal = false
				if(element !== null){
					if('internal' in pages && 'is_attribute' in pages.internal) {
						internal = element.hasAttribute(pages.internal.is_attribute)
					}
				}
				return internal
			}
	...
	class DomApp extends DomModule{
		static get pages(){
			return {
				...
				folder:'pages/',
				internal:{ is_attribute:'internal-page' },
				selector:'name',
				start:'welcome',
				suffix:'page'
			}
		...
		get_page(name){ return get_app_page(this,name) }
		get_page_element(name){ return this.query(AppPage.selector(this,name)) }
		...
		link_selector(e){
			...
			switch(type){
				case 'iron-select':
					...
					this.scroll_to_section(item.getAttribute('name'))
		...
		ready(){
			...
			window.setTimeout(()=>this.page_did_load(),100)
			
		}
		route_page(page) { return route_app_page(this,page) }
		
		page_changed(page_name){
			let page = this.get_page(page_name)
			if(page.internal) return this.page_did_load()
			return Polymer.importHref(page.url, this.page_did_load.bind(this), this.page_did_not_load.bind(this), true)
		}
		page_did_load(){
			...
			if(this.page === 'welcome') this.scroll_to_section()
		}
		scroll_to_section(name){
			let section_name = name || (window.location.hash ? window.location.hash.replace('#',''):'')
			
			if(section_name){
				let section = this.query(`section#${section_name}`)
				if(section) section.scrollIntoView()
			}
		}
		page_did_not_load() { this.page = this.constructor.pages.error_page || 'not-found' }
	...
	function get_app_page(element,name){ return new AppPage(name,element) }
	...
	function route_app_page(element,page_name){
		
		if (page_name === undefined) return element
		let link = element.opened_page_link
		if(link) link.setAttribute('aria-selected', 'false')
		element.page = page_name ? page_name:get_first_page()
		if(element.$.drawer && !element.$.drawer.persistent) element.$.drawer.close()
		...
		function get_first_page(){
			let pages = element.constructor.pages
			if('start' in pages) return pages.start
			pages = element.interface.pages
			if(pages.length) return pages[0].getAttribute('name')
			return null
		}
	...
	function set_app_page_links(element) {
		...
		function add_page_link(link){
			let before = null
			if('append_before_element' in link) before = link_container.querySelector(`[name="${link.append_before_element}"]`)
		...
		function create_page_link({before, label, name, section, page}) {
			...
			if(before) link.append_before_element = before
